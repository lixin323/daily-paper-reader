...
score: 6.0
evidence: 在遥操作中使用强化学习策略，服务于具身AI
tldr: "全身遥操作依赖外骨骼或多摄像头，成本高且环境受限；基于XR的端到端方法需重训练且忽略动态可行性。提出分层框架，用单个XR设备，通过MPC重定向联合优化操作意图与动态可行性，引入状态同步和SLAM位姿反馈。在类人机器人上完成时间降低30%以上、功耗降低20%，移动操作器零碰撞，真实实验验证有效性。该形态无关、即插即用的方案支持实时行为定制，可广泛部署。"
source: arxiv
...
motivation: 现有全身遥操作成本高或需机器人特定重训练，且运动重定向忽略动态可行性。
method: 提出分层框架，基于单个XR设备，用MPC重定向联合优化操作者意图与机器人动态可行性，引入状态同步和SLAM反馈。
result: "模拟中类人机器人完成时间降低30%以上、功耗降低20%，移动操作器零碰撞；真实实验验证跨形态有效性。"
conclusion: 该形态无关的即插即用框架实现稳定高效的全身遥操作，支持实时行为定制与广泛部署。
---

## 摘要
全身遥操作对于在移动操作任务中实现可扩展的机器人数据采集至关重要，然而现有依赖外骨骼套装或多摄像头设置的方法带来了高昂的成本、复杂性和环境限制。近期使用单一扩展现实（XR）设备与端到端强化学习策略的方法部分解决了这些限制，但需要针对特定机器人重新训练，存在分布外失败的问题，且依赖忽略动态可行性的运动重定向。我们提出一种由单一XR设备驱动的分层全身遥操作框架，该框架无需重新训练特定机器人策略即可泛化到多种机器人形态。基于模型预测控制（MPC）的运动重定向器联合优化与操作者意图的对齐以及机器人的动态可行性，为现有底层控制器生成最优指令。为确保稳健的在线执行，我们引入一种状态同步方法，在每个MPC步骤重置仿真器状态以处理含噪声的真实世界测量和接触敏感性，并集成基于SLAM的全局位姿反馈以缓解长期漂移。仿真结果表明，在全身控制任务中，人形机器人（完成时间降低30%以上，功耗降低20%）和移动操作臂（零碰撞）相比基线方法具有更高的成功率。真实世界实验进一步验证了我们方法的有效性和灵活性，展示了所提出的重定向器在两个平台上成功部署于全身控制任务，并允许用户根据偏好轻松调整遥操作行为。这种即插即用框架为全身机器人遥操作提供了一种可扩展、形态无关的解决方案，实现了实时行为定制和跨平台的广泛适用性。
