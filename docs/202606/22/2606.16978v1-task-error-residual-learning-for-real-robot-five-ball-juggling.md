---
title: Task-Error Residual Learning for Real-Robot Five-Ball Juggling
title_zh: 面向真实机器人五球杂耍的任务误差残差学习
authors: "Kai Ploeger, Jan Peters"
date: 2026-06-15
pdf: "https://arxiv.org/pdf/2606.16978v1"
tags: ["query:embodied-ai"]
score: 7.0
evidence: 基于强化学习洞察的残差学习用于真实机器人抛球
tldr: 为解决真实机器人五球杂耍中残差学习样本效率低的问题，本文提出以方向性任务误差替代标量奖励作为监督信号，并引入任务误差模型指导样本选择。在Barrett WAM机械臂上，从第二次尝试起即收敛，首次失败后任务误差单调下降不再丢球。相比通常需要数年练习的人类，该方法仅需极少量样本。实验表明监督信号的信息含量和学习器的利用方式比周围堆栈的精度更重要，最简单的固定雅可比牛顿更新最可靠。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2606.16978v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 578, \"height\": 898, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2606.16978v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1276, \"height\": 681, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2606.16978v1/fig-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1258, \"height\": 495, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2606.16978v1/fig-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1261, \"height\": 788, \"label\": \"Figure\"}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2606.16978v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1270, \"height\": 391, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2606.16978v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1195, \"height\": 184, \"label\": \"Table\"}]"
motivation: 标量奖励信息量少导致残差学习样本效率低，需充分利用任务误差的方向信息。
method: 采用方向性任务误差监督和任务误差模型驱动样本选择的残差学习方法。
result: 在真实机器人上从第二次尝试稳定完成五球杂耍，误差单调下降，无需失败。
conclusion: 监督信号的信息含量和学习器利用方式比堆栈精度更关键，固定雅可比牛顿更新最可靠。
---

## 摘要
对于优化现有行为的残差学习，样本效率取决于两个方面：每次尝试返回的信息量，以及学习器使用这些信息的效率。强化学习的标准标量奖励所携带的信息远少于定义任务的定向任务误差。随机探索进一步丢弃了每次尝试返回的信息。通过使用定向任务误差监督的残差学习和驱动样本选择的任务误差模型，我们在类人Barrett WAM机械臂上实现了稳定的三球、四球和五球杂耍。尽管通过一个简单、理想化的堆栈进行规划和控制，系统从第二次尝试开始收敛。第一次尝试失败后，任务误差单调递减，再无失败。相比之下，五球杂耍通常需要人类数年的练习。我们在三个二元轴上比较了残差学习器：学习反馈中的定向信息、分析先验的承诺，涵盖了牛顿型雅可比更新、复合贝叶斯优化和随机搜索方法。这两个轴都被证明是必要的：定向反馈和信息丰富的先验单独都不够，而结合两者的最简单方法——固定雅可比牛顿更新——最可靠。学习到的残差能够容忍较大的先验偏差和退化的关节跟踪，主要影响收敛速度。因此，真实机器人上残差学习的瓶颈在于监督信号的信息内容以及学习器如何使用它，而非周围堆栈的准确性。所有实验的视频文档见 https://kai-ploeger.com/residual-juggling。

## Abstract
For residual learning that refines existing behavior, sample efficiency depends on two things: how much information each rollout returns, and how efficiently the learner uses that information. Reinforcement learning's standard scalar reward carries far less information than the directional task error that defines the task. Random exploration further discards whatever information each rollout returns. Through residual learning with directional task-error supervision and a task error model that drives sample selection, we achieve stable three-, four-, and five-ball juggling on anthropomorphic Barrett WAM arms. Despite planning and controlling through a simple, idealized stack, the system converges from the second attempt. The first attempt drops, after which task error decreases monotonically without further failures. In comparison, five-ball juggling typically takes humans years of practice. We compare residual learners across two ternary axes, the directional information in the learning feedback and the commitment of the analytic prior, spanning Newton-style Jacobian updates, Composite Bayesian Optimization, and stochastic search methods. Both axes prove necessary: neither directional feedback nor an informative prior suffices alone, and the simplest method that combines them, a fixed-Jacobian Newton update, is the most reliable. The learned residual tolerates substantial prior misalignment and degraded joint tracking, affecting mainly convergence speed. The bottleneck for residual learning on real robots is therefore the information content of the supervision signal and how the learner uses it, not the accuracy of the surrounding stack. Video documentation of all experiments is available at https://kai-ploeger.com/residual-juggling.