...
title_zh: 强化学习引导的软融合检索：缺失模态下的鲁棒多模态模仿学习
...
tldr: 机器人多模态模仿学习面临传感器故障或遮挡导致的模态缺失问题。本文提出RL4IL，利用强化学习策略从训练库中检索最相关演示，通过软交叉注意力融合预测动作；缺失模态时，通过软插补头重构缺失嵌入，无需重新训练。在LIBERO基准测试中，RL4IL在传感器丢失条件下显著优于现有方法，提供了鲁棒的解决方案。
source: arxiv
...
motivation: 机器人传感器在部署中可能故障或遮挡，导致模态缺失，而现有模仿学习缺乏鲁棒性。
method: RL4IL使用PPO训练检索策略，基于BFS候选集排序演示，软交叉注意力融合动作；缺失模态时，通过软插补头重构嵌入。
result: 在三个LIBERO套件上，RL4IL在传感器缺失条件下大幅超越SOTA模仿学习方法，且无需训练策略网络。
conclusion: RL4IL有效处理模态缺失，无需重训练即可实现鲁棒的多模态模仿学习。
---

## 摘要
机器人系统通过多种输入模态（包括视觉相机流和自然语言指令）感知世界，并必须根据这些信号选择适当的动作。然而，假设所有输入设备永久可用是不现实的，因为传感器在部署过程中可能发生故障、被遮挡或完全掉线。因此，鲁棒地处理此类缺失模态场景对于真实世界的机器人操作至关重要。本文提出RL4IL，一种强化学习引导的模仿学习方法，通过从训练库中识别最相关的专家演示，为给定观测选择最合适的动作。通过近端策略优化在广度优先搜索候选集上训练的强化学习策略对候选演示进行排序，软交叉注意力融合头聚合它们的动作信号以产生最终预测。当推理时某个模态缺失时，专用的逐模态RL检索策略从训练库中识别供体演示，软插补头通过对排名靠前的供体进行交叉注意力来重建缺失嵌入——无需重新训练系统。在三个LIBERO基准套件上的实验表明，RL4IL在传感器掉线条件下显著优于最先进的模仿学习方法，且无需策略网络训练。代码可在https://github.com/h-ismkhan/Reinforcement-Learning-via-kNN-for-Robotic-Learning-with-Missing-Camera找到。
